...
function init() {
	...
	var checkBoxes = $('managePagesTable').getElements('input[id^=module]');
	do_check = false;
	...
	if ($('delete')) {
		$('delete').addEvent('click', function(el){
			...
			$('delete').setProperty('href', '/cms/administration/module/deletemoduleaction/?moduleList=' + checkedString);
		});
	}
}
...
var checkItem = function(el,checked) {
	itemID = el.getProperty('id').replace('module','');
