...
function init() {
	var checkBoxes = $('managePagesTable').getElements('input[id^=survey]');
	do_check = false;
	...
	if ($('delete')) {
		$('delete').addEvent('click', function(el){
			...
			$('delete').setProperty('href', '/cms/content/survey/actiondeletesurvey/?surveyList=' + checkedString);
		});
	}
	
	if ($('deleterelation')) {
		$('deleterelation').addEvent('click', function(el){
			checkedString = checkedArray.toString();
			$('deleterelation').setProperty('href', '/cms/content/survey/deleterelation/?qrelationidList=' + checkedString);
		});
	}
	...
	if ($('copy')) {
		$('copy').addEvent('click', function(el){
			checkedString = checkedArray.toString();
			$('copy').setProperty('href', '/cms/content/survey/copy/?surveyList=' + checkedString);
		});
	}
}
	...
	var checkItem = function(el,checked) {
		itemID = el.getProperty('id').replace('survey','');
...
//		console.log("Is Item %s checked? %d.",itemID,checked);
